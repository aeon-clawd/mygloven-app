...
import { TAG_OPTIONS, type ArtistImage } from "./types";

interface PhotosSectionProps {
  images: ArtistImage[];
  onChange: (images: ArtistImage[]) => void;
...
export function PhotosSection({ images, onChange, showWarnings = true }: PhotosSectionProps) {
  ...
  if (showWarnings) {
    if (images.length > 0 && images.length < 2) warnings.push("Mínimo 2 fotos para activar el artista");
